...
const TravelForm = ({ handleAnimate, setPlanTripFromGeminiJson }) => {
  ...
  const validateStepOne = () => {
    ...
    if (
      ...
      errors.dateRange = "Travel dates are required.";
  ...
  const validateStepTwo = () => {
    ...
    if (
      (!formData.customBudget.trim() ||
        isNaN(Number(formData.customBudget)) ||
        Number(formData.customBudget) <= 0) &&
      !formData.selectedBudgetOption
    ) {
      errors.customBudget = "Budget must be a positive number.";
    }
    if (!formData.selectedBudgetOption && !formData.customBudget)
      errors.selectedBudgetOption = "Please select a budget options";
  ...
  const validateStepThree = () => {
    ...
    if (!formData.travelWith)
      errors.travelWith = "Please select who you are traveling with.";
  ...
  const validateStepFour = () => {
    ...
    if (!formData.tripTheme)
      errors.tripTheme = "Please select a theme for your trip.";
  ...
  const handleSubmit = (e) => {
    ...
    const prompt = {
      prompt: `Plan a trip to ${formData.location} from ${
        ...
        formData.tripTheme
      }. Each day should include at least 3 activities and keep activity name max two words and give me actual hotel name.`,
    };
          ...
          {step === 2 && (
            <>
              ...
              <div className="mb-6">
                ...
                >
                  {["Low Budget", "Medium Budget", "High Budget"].map(
                    (option) => (
                      <div
                        key={option}
                      ...
                      >
                        {option === "Low Budget" && (
                          <FaWallet className="text-2xl mb-2 text-blue-500" />
                        )}
                        {option === "Medium Budget" && (
                          <FaDollarSign className="text-2xl mb-2 text-green-500" />
                        )}
                        {option === "High Budget" && (
                          <FaMoneyBillWave className="text-2xl mb-2 text-purple-500" />
                        )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    >
                      ...
                      {option === "Couple" && (
                        <FaHeart className="text-2xl mb-2 text-pink-500" />
                      )}
                      {option === "Colleagues" && (
                        <FaBriefcase className="text-2xl mb-2 text-yellow-500" />
                      )}
                      {option === "Group Tour" && (
                        <FaGlobe className="text-2xl mb-2 text-teal-500" />
                      )}
                      <span className="font-medium">{option}</span>
                    </div>
                  ))}
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[
                    {
                      label: "Honeymoon",
                      icon: <FaHeart className="text-2xl mb-2 text-pink-500" />,
                    },
                    {
                      label: "Family Trip",
                      icon: (
                    ...
                    },
                    {
                      label: "Business Trip",
                      icon: (
                        <FaBriefcase className="text-2xl mb-2 text-gray-700" />
                      ),
                    },
                    {
                      label: "Summer Vacation",
                      icon: <FaSun className="text-2xl mb-2 text-yellow-500" />,
                    },
                    {
                      label: "Road Trip",
                      icon: <FaCar className="text-2xl mb-2 text-blue-500" />,
                    },
                    {
                      label: "Backpacking",
                      icon: (
                        <FaSuitcaseRolling className="text-2xl mb-2 text-green-500" />
                      ),
                    },
                    {
                      label: "Festival/Party",
                      icon: (
                        <FaGlassCheers className="text-2xl mb-2 text-orange-500" />
                      ),
                    },
                    {
                      label: "Photography Tour",
                      icon: <FaCamera className="text-2xl mb-2 text-red-500" />,
                    },
                    {
                      label: "Cruise",
                      icon: <FaShip className="text-2xl mb-2 text-teal-500" />,
                    },
                    {
                      label: "Wildlife Safari",
                      icon: (
                        <FaBinoculars className="text-2xl mb-2 text-brown-500" />
                      ),
                    },
                    {
                      label: "Ski Trip",
                      icon: <FaSkiing className="text-2xl mb-2 text-sky-500" />,
                    },
                    {
                      label: "Food Tour",
                      icon: (
                        <FaUtensils className="text-2xl mb-2 text-red-600" />
                      ),
                    },
                  ].map((theme) => (
                    <div
        ...
        <div className="flex justify-center gap-8 mt-8">
          {step > 1 && (
            <Button
              onClick={prevStep}
              type="button"
              className="px-6 py-2 bg-gray-400 hover:bg-gray-500 text-white rounded-lg hover:scale-105 transition"
            >
              Back
            </Button>
          )}
          {step < 4 && (
            <Button
              onClick={nextStep}
              type="button"
              className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg hover:scale-105 transition"
            >
              Next
            </Button>
          )}
          {step === 4 && (
            <Button
              type="submit"
              className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg hover:scale-105 transition"
            >
              Create Trip
            </Button>
          )}
